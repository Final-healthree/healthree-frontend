...
const MainGoalThird = (props) => {
  ...
  const selectedDay = new Date(
    new Date(date).getTime() + (24 - 9) * 60 * 60 * 1000
  );
  const videoUploadCheck1 = useSelector(
    (state) => state.certification.list.result?.day1
  );
  const videoUploadCheck2 = useSelector(
    (state) => state.certification.list.result?.day2
  );
  const videoUploadCheck = useSelector(
    (state) => state.certification.list.result?.day3
  );
  ...
  return (
    <StMainLayout>
      ...
      {videoUploadCheck1?.uploaded === true &&
      videoUploadCheck2?.uploaded === true &&
      videoUploadCheck?.uploaded === false ? (
        today < selectedDay ? (
          ""
        ) : failmodalClose === true ? (
          <FailModal
            number={3}
            date={getMainGoal?.day3.date.slice(0, 10)}
      ...
      {videoUploadCheck?.uploaded === false ? (
        ...
        <MainModal number={3} date={getMainGoal?.day1.date.slice(0, 10)} />
      ) : null}
